...
FinalDF.to_csv("../data/output/all_Excel_FinalDF_matches_2018-19_032920_v4_TEST.csv", header=True, index=False)
...
for index, row in FinalDF.iterrows():
      if index % 2 == 0:
         ...
         if row['Team'] == "USA":
             ...
             #### Fix, 3/28/20 ################
             # Check for zero to avoid 'division by zero' errors
             # Pen-FK Diff
             if (row['Pens_Frees_Against']) == 0:
                 PenFkUSA == 0
             elif (new_row['Pens_Frees_Against']) == 0:
                 PenFkOpp == 0
             else:
                 TotPenFk = (row['Pens_Frees_Against']) + (new_row['Pens_Frees_Against'])
                 PenFkUSA = (new_row['Pens_Frees_Against']*100)/TotPenFk
                 PenFkOpp = (row['Pens_Frees_Against']*100)/TotPenFk
...
# =============================================================================
             ...
             # Cards with a factor of -50 applied to reduce the impact on model
             ...
             if (row['Scores']) == 0:
                 ScoresOpp == 0
             elif (new_row['Scores']) == 0:
                 ScoresUSA == 0
             else:
                 TotScores = (row['Scores']) + (new_row['Scores'])
                 ScoresUSA = (new_row['Scores']*100)/TotScores
                 ScoresOpp = (row['Scores']*100)/TotScores
             ...
             # == check for fix in USA rows in line 235
             # Pen-FK
             TotPenFk = float(row['Pens_Frees_Against']) + float(new_row['Pens_Frees_Against'])
             PenFkUSA = float(new_row['Pens_Frees_Against']*100)/TotPenFk
             PenFkOpp = float(row['Pens_Frees_Against']*100)/TotPenFk
             PenFk_diff = PenFkUSA - PenFkOpp
...
#sub.to_csv('../data/output/2018-19_diffs.csv', mode='a', header=True, index=False)
sub.to_csv('../data/output/final_diffs_all_2018-19_Final_032920_v4.csv', header=True, index=False)