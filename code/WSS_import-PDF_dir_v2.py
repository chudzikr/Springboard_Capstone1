...
# iterate through the rows of the dataframe and get the next row relative to the 
#current row
...
# =============================================================================
# ROB
# Need to order rows by tournament & match date?
# First, subtract the entire row, THEN calculate percentages, etc?
# =============================================================================

# Sort dataframe to ensure matches are in ordered pairs (w/ USA & Opp)
#df.sort_values(by=['col1', 'col2'])
FinalDF.sort_values(by=['Tournament', 'Match'], inplace=True)

# =============================================================================
# ROB Edits - subtraction operation on entire rows
# =============================================================================
...
for index, row in FinalDF.iterrows():
     if index % 2 == 0:
         ...
         if row['Team'] == "USA":
             ...
             FinalDF['PossessionTimeRelativeUSA'] = PossessionUSA - PossessionOpp
             
             TotalScores = float(row['Scores']) + float(new_row['Scores'])
             ScoreUSA = float(row['Scores'] *100)/TotalScores
             ScoreOpp = float(new_row['Scores'] *100)/TotalScores
             FinalDF['ScoreRelativeUSA'] = ScoreUSA - ScoreOpp
         else: 
             new_row['Team'] == 'USA'
             TotPossTime = float(row['Possession Time']) + float(new_row['Possession Time'])
             ...
             TotalScores = float(row['Scores']) + float(new_row['Scores'])
             ScoreUSA = float(new_row['Scores'] *100)/TotalScores
             ScoreOpp = float(row['Scores'] *100)/TotalScores
             FinalDF['ScoreRelativeUSA'] = ScoreUSA - ScoreOpp
...
#         new_row = FinalDF.iloc[index+1]
#         if row['Team'] == "USA":
#             # (row - new_row) -- BUT ONLY NUMERIC ROWS - Cols 4:20
#             diffDF.append((row[4:20] - new_row[4:20]), ignore_index=True)
#             #diffDF.append(row - new_row) # FIGURE OUT HOW TO BUILD ROW WITH TEAM/MATCH/ETC VALUES + SUBTRACTED COLUMN VALUES
#             # insert new row via pd.append() , keep 'Team' name from new_row
#         if new_row['Team'] == 'USA':
#             # (new_row - row) ---- BUT ONLY NUMERIC ROWS - Cols 4:20
#             diffDF.append((new_row[4:20] - row[4:20]), ignore_index=True)
#             # insert new row via pd.append() , keep 'Team' name from new_row
# =============================================================================
